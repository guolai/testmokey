...
//                        @"    is object value");
    ...
    case _C_CHARPTR: /* FIXME: check if this is good (copy/no copy)*/
            (*((const char **)value)) = [[anObject stringValue] cStringUsingEncoding:NSASCIIStringEncoding];
            NSDebugLLog(@"STStructure",
...
+ invocationWithTarget:(id)target selectorName:(NSString *)selectorName
{
    ...
    if(!sel)
    {
        ...
        const char *name = [selectorName cStringUsingEncoding:NSASCIIStringEncoding];
    ...
    if(requiresRegistration)
    {
        ...
        sel = ObjcRegisterSel([selectorName cStringUsingEncoding:NSASCIIStringEncoding], [signature methodReturnType]);
...
- (void)setArgumentAsObject:(id)anObject atIndex:(int)anIndex
{
    ...
    value = NSZoneMalloc(NSDefaultMallocZone(), ObjcSizeOfType(type));

    STGetValueOfTypeFromObject(value, type, anObject);

    [self setArgument:(void *)value atIndex:anIndex];
    NSZoneFree(NSDefaultMallocZone(),value);
}
...
//               @"  return type '%s', buffer length %i",type,returnLength);
    ...
    if(returnLength!=0)
    {
        value = NSZoneMalloc(NSDefaultMallocZone(),returnLength);
